...
# count ? count = var.create_budget_alarm ? 1(true) :0(false) for overall Call
  dynamic "cost_types" {
...
terraform {
  ...
  backend "s3" {
    bucket         = "terraform-wm-backend-raghav"
    key            = "wm_tasks/terraform.tfstate"
    region         = "us-east-1"
...
# python operator "hello world kubebuilder framework"